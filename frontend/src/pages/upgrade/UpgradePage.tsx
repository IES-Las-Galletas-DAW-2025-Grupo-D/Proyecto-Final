            ...
            <div className="card-body">
              ...
              <div className="card-actions mt-6">
                <button 
                  ...
                  onClick={() => handlePurchase(environment.INTERMEDIATE_PRICE_ID)}
                  disabled={isLoading}
                >
                  {isLoading ? <span className="loading loading-spinner"></span> : 'Get Started'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="card bg-base-200 shadow-xl border border-base-300 text-neutral">
            <div className="card-body">
              ...
              <div className="card-actions mt-6">
                <button 
                  className={`btn btn-primary w-full ${isLoading ? 'loading' : ''}`}
                  onClick={() => handlePurchase(environment.BUSINESS_PRICE_ID)}
                  disabled={isLoading}
                >
                  {isLoading ? <span className="loading loading-spinner"></span> : 'Get Started'}
                </button>
